package be.maxouuuuu.spigotmc.utils.reflexion;
...
public class Reflection {
	...
	public static Class<?> getCbClass(String name) {
		String className = "org.bungee.craftbukkit." + getVersion() + "." + name;
	...
	public static Field getOnlyField(Class<?> toGetFrom, Class<?> type) {
		...
		for (Field field : toGetFrom.getDeclaredFields()) {
			if (!type.isAssignableFrom(field.getClass()))
				continue;
			Preconditions.checkArgument(only == null, "More than one field of type %s on %s: %s and %s",
					type.getSimpleName(), toGetFrom.getSimpleName(), field.getName(), only.getName());
			only = field;
		}
		return only;
	}
	...
	public static Method getOnlyMethod(Class<?> toGetFrom, Class<?> returnType, Class<?>... paramSpec) {
		...
		for (Method method : toGetFrom.getDeclaredMethods()) {
			...
			if (!isParamsMatchSpec(method.getParameterTypes(), paramSpec))
				continue;
			Preconditions.checkArgument(only == null, "More than one method matching spec on %s"
					+ ((only.getName().equals(method.getName())) ? "" : ": " + only.getName() + " " + method.getName()),
					toGetFrom.getSimpleName());
			only = method;
		}
		return only;
	}
