...
public class LagCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command arg1, String arg2, String[] args) {
		if (!(sender instanceof Player)) {
			sender.sendMessage(easyLag.getMessage("message.sender_isnt_player"));
			return false;
		}
		...
		if (args.length == 0) {
			...
			if(easyLag.getMessage("main.use_24_hour_format").equals("true")) {
				player.sendMessage(easyLag.getMessage("format.first_line").replace("$server_name$", Bukkit.getServerName()).replace("$date$", date).replace("$time$", time24));
			} else {
				String dateUK = inFormat.format(new Date()).toString();
				player.sendMessage(easyLag.getMessage("format.first_line").replace("$server_name$", Bukkit.getServerName()).replace("$date$", date).replace("$time$", dateUK));
			}
